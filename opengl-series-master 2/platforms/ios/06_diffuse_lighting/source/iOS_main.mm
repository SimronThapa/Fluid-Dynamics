...
glm::vec2 gDragPoint(0,0);
tdogl::Camera gCamera;
ModelAsset gWoodenCrate;
std::list<ModelInstance> gInstances;
GLfloat gDegreesRotated = 0.0f;
Light gLight;
Gesture gGesture = {0, 0, eNone, false};
float gMoveSpeed = DEFAULT_MOVE_SPEED; //units per second
...
void Update(float secondsElapsed) {
	...
	switch(gGesture.action){
		case eW:{
			if(gGesture.continious){
				gCamera.offsetPosition(secondsElapsed * gMoveSpeed * glm::vec3(0,1,0));
			}else{
		...
		case eA:{
			if(gGesture.continious){
				gCamera.offsetPosition(secondsElapsed * gMoveSpeed * -gCamera.right());
			}else{
		...
		case eZoomIn:{
			gCamera.offsetPosition(secondsElapsed * gMoveSpeed * gCamera.forward());
		}	break;
		...
		case eDrag:{
			...
			gCamera.offsetOrientation(mouseSensitivity * gDragPoint.y, mouseSensitivity * gDragPoint.x);
		}	break;
